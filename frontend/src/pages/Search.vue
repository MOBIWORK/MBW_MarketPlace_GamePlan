<template>
  ...
  <div class="mx-auto mt-6 max-w-4xl px-4 sm:px-5">
    <div class="flex items-center space-x-2">
      ...
      <Button @click="search(query)" :loading="$resources.search.loading"> {{__('Search')}} </Button>
    </div>
    <div
      v-if="$resources.search.params && $resources.search.data"
      class="mt-4 text-base font-semibold text-ink-gray-8"
    >
      {{__('About')}} {{ $resources.search.data.total }} {{__('results for')}} "{{ $resources.search.params?.query }}"
