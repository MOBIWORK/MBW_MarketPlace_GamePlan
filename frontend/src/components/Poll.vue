<template>
  <div class="py-6 transition-shadow" :class="{ ring: highlight }">
    ...
    <div class="mt-1 text-sm text-ink-gray-5">
      ...
      <span> {{ _poll.total_votes }} {{ _poll.total_votes === 1 ? __('vote') : __('votes') }} </span>
    ...
    <Dialog :options="{ title: __('Poll results') }" v-model="showDialog" v-if="pollResults">
    ...
    submitVote(option) {
      if (this._poll.anonymous) {
        this.$dialog({
          ...
          message: `${__("This poll is anonymous. Once you vote, you cannot retract your vote. You are voting for")} "${option.title}". ${__("Continue")}?`,
          actions: [
            {
              label: `${__("Vote for")} "${option.title}"`,
              variant: 'solid',
              onClick: (close) => {
