...
def get_user_info(user=None):
    ...
    roles = frappe.db.get_all(
        "Has Role", filters={"parenttype": "User"}, fields=["role", "parent"])
...
def invite_by_email(emails: str, role: str, projects: list = None):
    ...
    if role == "Gameplan Guest":
        ...
        to_invite = list(set(email_list) -
                         set(existing_members) - set(existing_invites))
...
def recent_projects():
    ...
    pinned_projects_query = frappe.qb.from_(Pin).select(
        Pin.project).where(Pin.user == frappe.session.user)
    projects = (
...
def active_projects():
    ...
    active_projects_comment_count = {
        d.project: d.comments_count for d in active_projects}
    for d in projects:
...
def search(query, start=0):
    ...
    discussion_names = [d.payload["reference_name"]
                        for d in comments_by_doctype.get("GP Discussion", [])]
    task_names = [d.payload["reference_name"]
                  for d in comments_by_doctype.get("GP Task", [])]
